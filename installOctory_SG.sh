...
webresources="https://github.com/jfbutkiewicz/SG-Octory/raw/refs/heads/main/Resources.zip"
onboarding="https://github.com/jfbutkiewicz/SG-Octory/raw/refs/heads/main/onboarding.plist"
pkgresources="https://github.com/jfbutkiewicz/SG-Octory/raw/refs/heads/main/Octory-3.0.0.pkg"
notifierresources="https://github.com/jfbutkiewicz/SG-Octory/raw/refs/heads/main/Octory_Notifier_1.0.1.pkg"

# Standard Variables
targetdir="/Library/Application Support/Octory"                 # Installation directory
appname="Octory for SG"                                                # Name of application to display in the logs
logandmetadir="/Library/Logs/Microsoft/IntuneScripts/Octory"    # Log file directory

# Generated variables
tempdir=$(mktemp -d /tmp/Octory.XXXX)                                          # Temp directory
# Temp file
resfile="$tempdir/Resources.zip"
pkgfile="$tempdir/Octory-3.0.0.pkg"
notifierfile="$tempdir/Octory Notifier_1.0.1.pkg"
onboardingfile="$tempdir/onboarding.plist"
...
chmod -R a+r $tempdir
# Download Octory
echo "$(date) | temp folder set to $tempdir"
echo "$(date) | Downloading resources files from [$webresources]"
curl -f -s --connect-timeout 10 --retry 3 --retry-delay 10 -L -ko "$resfile" "$webresources"
echo "$(date) | Downloading configuration files from [$onboarding]"
curl -f -s --connect-timeout 10 --retry 3 --retry-delay 10 -L -ko "$onboardingfile" "$onboarding"
echo "$(date) | Downloading octory package files from [$pkgresources]"
curl -f -s --connect-timeout 10 --retry 3 --retry-delay 10 -L -ko "$pkgfile" "$pkgresources"
echo "$(date) | Downloading octory package files from [$notifierresources]"
curl -f -s --connect-timeout 10 --retry 3 --retry-delay 10 -L -ko "$notifierfile" "$notifierresources"
...
chmod 644 $onboardingfile
unzip -q $resfile
sudo installer -pkg "$pkgfile" -target /
if [[ $? -eq 0 ]]; then
    echo "$(date) | octory succesfully installed"
    else
    echo "$(date) | octory installation failed"
fi

# Install octo-notifier
sudo installer -pkg "$notifierfile" -target /
if [[ $? -eq 0 ]]; then
...
md Octory
mv $onboardingfile ./Octory/
mv $tempdir/Resources ./Octory
